...
class SlackApp:
    ...
    def get_client(self) -> SlackClient:
        ...
        if "Item" not in auth_token:
            log.warning(
                f"can't notify slack team {self.team_id} of response - missing auth token"
            )
            return None
    ...
    def get_auth_token(self) -> Optional[Dict]:
        return qanda.table.auth_token.get_item(Key={"id": self.team_id})["Item"]
    ...
    def handle_message_event(self, evt):
        ...
        def reply(**kwargs):
            client.api_call("chat.postMessage", channel=channel_id, **kwargs)
        ...
        if global_enabled and subscription:
        ...
        if bodylc.startswith("subscribe"):
            ...
            reply(
                text=f'Ok! You\'ll get notifed of new questions. Message me "unsubscribe" at any time to shut me up {LOGO}'
            )
            log.info("new slack subscriber!")

        elif bodylc.startswith("global") and global_enabled:
            if not subscription:
                return reply(
                    text="You're not subscribed to receive messages right now."
                )
            ...
            reply(
                text=f"Awesome! Now you'll get all questions, regardless of their origin. :world_map:\nBe prepared to see some offensive and terrible things.\nReply *local* to return to the hugbox of {team_name} at any time."
            )

        elif bodylc.startswith("local") and global_enabled:
            if not subscription:
                return reply(
                    text="You're not subscribed to receive messages right now."
                )
            ...
            qanda.table.subscriber.put_item(Item=subscription)
            reply(
                text=f"Alrightie! You will now only get questions from {team_name}.\nReply *global* to get messages from anywhere."
            )
            ...
            if ok:
                reply(
                    text=f"{LOGO} Thanks! Your answer's been anonymously sent to the asker."
                )
            else:
                reply(
                    text="So sorry... I don't have any record of asking you a question."
                )
            ...
            reply(
                text=f"So sorry.. not sure what you're asking {LOGO}\n{USAGE}\n\n{cross_slack_status_msg}"
            )
    ...
    def get_question_actions(self, notified: int, qid: str):
        actions = [
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": "Would you like to post your question publicly <on the web?|https://appqanda.com>",
                },
            },
            {
                "type": "actions",
        ...
        if notified:
            actions.insert(
                0,
                {
                    "type": "section",
                    "text": {
                        "text": "{LOGO} Splendid! Your anonymous question has been sent out to {notified} people.\nI'll message you with the answers."
                    },
                },
            )
