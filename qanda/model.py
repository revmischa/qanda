...
class Model:
    ...
    def id_and_created(self, id_=None):
        if id_ is None:
            id_ = self.make_id()
        return dict(id=id_, created=int(time.time()))
    ...
    def get_questions_by_id(self, question_ids: List[str]) -> List[Dict]:
        ...
        res = self.client.batch_get_item(
            RequestItems={"qanda.question": {"Keys": keys}}
        )
    ...
    def save_slack_tokens(self, token_res):
        ...
        log.debug(token_res)
        self.auth_token.put_item(
            Item=dict(**token_res, **self.id_and_created(id_=team_id))
        )
    ...
    def new_message(
        ...
        **kwargs,
        ...
        msg = {"from": from_, "to": to_, "sid": sid, **self.id_and_created(), **kwargs}
        if body:
            msg["body"] = body
        if question_id:
    ...
    def new_question_from_web(self, body: str, remote_ip: str = None) -> Question:
        """Record and publish question asked from web form."""
        # record question
        q = dict(body=body, remote_ip=remote_ip, **self.id_and_created(), source="web")
        self.question.put_item(Item=q)
        return q

    def new_question_from_slack(
        self,
        body: str,
        channel_id: str,
        user_id: str,
        ...
        **kwargs,
        ...
        # record question
        q = dict(body=body, **slack_params, **self.id_and_created())
        self.question.put_item(Item=q)
    ...
    def _find_question_message_to(self, to, filter_expression=None) -> Optional[Dict]:
        ...
        if not filter_expression:
            filter_expression = (
                Attr("question_id").exists() & Attr("is_question_notification").exists()
            )
        res = self.message.query(
            IndexName="to-created-index",  # FIXME: put in CF
            ScanIndexForward=False,  # give us most recent first
            KeyConditionExpression=Key("to").eq(
                to
            ),  # find message sent to this answerer
    ...
    def question_append_answer(self, question: Dict, answer: Dict) -> Dict:
        ...
        res = self.question.update_item(
            Key={"id": question["id"]},
            AttributeUpdates={"answers": {"Value": [answer], "Action": "ADD"}},
        )
        return res

    def new_answer_from_web(
        self, body: str, question: Dict, remote_ip: str = None
    ) -> Dict:
        answer = {
            **self.id_and_created(),
            "body": body,
    ...
    def new_answer_from_slack_pm(
        self, body: str, user_id: str, team_id: str, channel_id: str
    ) -> bool:
        ...
        filter_expression = (
            Attr("question_id").exists()
            & Attr("is_question_notification").exists()
            & Attr("slack_team_id").eq(team_id)
        )
        question_msg = self._find_question_message_to(
            to=channel_id, filter_expression=filter_expression
        )
        if not question_msg:
            log.warning(
                f"got slack response to question but no question found for {channel_id}"
            )
            return False
    ...
    def new_answer_from_sms(self, body: str, sid: str, from_: str, to_: str):
        answer_msg = self.new_message(
            body=body, sid=sid, from_=from_, to_=to_, is_answer=True, source="sms"
        )
        ...
        if not question_msg:
            ...
            log.warning(
                f"got SMS but no questions have been asked. from: {from_}. body: {body}"
            )
    ...
    def get_questions(self, source: str = None, start_key: Dict = None) -> Dict:
        ...
        if source:
            query_params["KeyConditionExpression"] = Key("source").eq(
                source
            )  # filter by source
        else:
            # all sources
